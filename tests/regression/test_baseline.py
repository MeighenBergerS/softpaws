...
def test_range_table_pinned(key, kwargs, function):
    ...
    The paper's rows were generated by an older commit and sit about 0.6 km
    away from these; the table is regenerated from the library before
    submission, so the refactor is held to today's values.
    """
    want = BASELINE["range_to_threshold_water_1tev"]
    energies = 10.0 ** np.asarray(want["log10_e"], dtype=float)
